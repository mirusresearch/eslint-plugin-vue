...
function definePropertyReferenceExtractor(
  ...
  /** @type {{ toRefNodes: Set<ESNode>, toRefsNodes: Set<ESNode>, stateToRefsNodes: Set<ESNode>} | null} */
  let toRefSet = null
  ...
  function getToRefSet() {
    ...
    const stateToRefsNodes = new Set()
    for (const { node } of tracker.iterateEsmReferences(
      utils.createCompositionApiTraceMap({
        [eslintUtils.ReferenceTracker.ESM]: true,
        stateToRefs: {
      ...
      stateToRefsNodes.add(node)
    }

    return (toRefSet = { toRefNodes, toRefsNodes, stateToRefsNodes })
  }
  ...
  function extractFromCall(node, argIndex) {
    ...
    if (!fnNode) {
      if (argIndex === 0) {
        ...
        } else if (getToRefSet().stateToRefsNodes.has(node)) {
          return extractFromStateToRefs(node)
        }
      }
      return {
        hasProperty(_name, options) {
  ...
  /**
   * Extract the property references from stateToRefs call.
   * @param {CallExpression} node
   * @returns {IPropertyReferences}
   */
  function extractFromStateToRefs(node) {
    const base = extractFromExpression(node, false)
    return {
      hasProperty: (name, option) => base.hasProperty(name, option),
      allProperties: () => base.allProperties(),
